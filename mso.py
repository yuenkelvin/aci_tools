...
class MsoRestClient(object):
    ...
    def __init__(self, server_endpoint, server_port, admin_user, admin_password, **kwargs):
        """
        ...
        self.protocol = 'https'
        self.bearer = self.__login(server_endpoint, server_port,  admin_user, admin_password)

    def __login(self, server_endpoint, server_port, admin_user, admin_password):
    ...
    def __msoLogMeIn(self, ip_addr, port, username, password):
        url = self.protocol + '://' + ip_addr + ':' + port + '/api/v1/auth/login'
        json_creds = '{"username" : "%s", "password" : "%s"}' % (username, password)
        mso_headers = {   'Host': ip_addr + ':' + port, \
                          'Origin': 'https://'+ip_addr + ':' + port, \
    ...
    def make_http_request(self, http_method, uri_path, args=None):
        ...
        unprep_req = requests.Request(
            http_method, self.protocol + '://' + self.server_endpoint + ':' + self.server_port + uri_path, params=params,
